...
# =======================
# base class to build model from dict
# (no np or jax yet)
# =======================
class wtf_base:
    def __init__(self, wtf_dict, to_array_func, to_array_dtype):
        ...
        # build lists of layers for stem and each branch
        self.stem_layers, self.branches = self.build_model_from_dict(to_array_func, to_array_dtype)

    # -----------------------
    # convert layer list (branch or stem) using chosen backend
    # -----------------------
    def build_layers_from_dict(self, layer_list, layer_names, to_array_func, to_array_dtype):
        """
        converts named layers from the dictionary into tuples containing parameters and activation functions.
        """
        for layer_name in layer_names:
            layer = self.wtf_dict['layers'][layer_name]
            layer_type = layer['type']

            # dense layer so store: w, b, act
            if layer_type == 'Dense':
            ...
            # input layer so store: batch shape
            elif layer_type == 'InputLayer':
                
                layer_list.append(('InputLayer', layer['batch_shape']))

    # -----------------------
    # build model structure from branch and stem lists
    # -----------------------
    def build_model_from_dict(self, to_array_func, to_array_dtype):
        """
        builds ordered layer lists for the model stem and branches
        """
        # build stem
        stem_layers = []
...
# =======================
# NumPy backend for forward pass
# =======================
class numpy_compile(wtf_base):
    ...
    def __init__(self, wtf_dict):
        ...
        # convert layers into a callable function for stem + branches
    ...
    # -----------------------
    # convert layers to callables
    # -----------------------
    def make_layer_functions(self, layers):
        """
        convert layer tuple into a callable
        """
        layer_functions = []
        for layer in layers:
            # dense -> act(x @ w + b)
        ...
        for layer in layers:
            ...
            if layer[0] == 'Dense':
                ...
                def layer_function(x, weights=weights, biases=biases, activation=activation):
                    ...
                    if activation == 'elu':
                        x = self.jax.nn.elu(x) # <- JAX in-line elu
                ...
                # skip shape check from NumPy version - JAX hates assert, slows down compile
                _, batch_shape = layer
